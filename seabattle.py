    ...
    def shot(self, shot_point, hid=False):
        try:
            if shot_point in self.shot_points or \
                    shot_point.x < 0 or shot_point.x > 6 or shot_point.y < 0 or shot_point.y > 6:
                raise IndexError
            ...
            if hid is False:
                print("Координаты выстрела некорректны, попробуйте еще раз")
...
class Game:
    def __init__(self, player=None, ai=None):
        ...
        self.ship_names = ("крейсер (3 клетки)", "эсминец 1 (2 клетки)", "эсминец 2 (2 клетки)",
                           "катер 1 (1 клетка)", "катер 2 (1 клетка)", "катер 3 (1 клетка)", "катер 4 (1 клетка)")
